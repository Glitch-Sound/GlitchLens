...
function renderHtml(model: VisualizationViewModel, nonce: string, viewId: string): string {
	...
	return [
		...
		`<script nonce="${nonce}">const WHEEL_ZOOM_FACTOR=0.0025;let pinchState;const activePointers=new Map();function pointerDistance(pointA,pointB){return Math.hypot(pointA.clientX-pointB.clientX,pointA.clientY-pointB.clientY);}function endPinch(){pinchState=undefined;activePointers.clear();document.body.style.removeProperty('user-select');}viewer?.addEventListener('wheel',event=>{if(isPanExcludedTarget(event.target))return;const delta=finiteOrZero(event.deltaY);if(delta===0)return;const factor=event.ctrlKey||event.metaKey?WHEEL_ZOOM_FACTOR*2:WHEEL_ZOOM_FACTOR;event.preventDefault();setZoom(currentScale*(1-delta*factor));},{passive:false});viewer?.addEventListener('pointerdown',event=>{if(isPanExcludedTarget(event.target))return;activePointers.set(event.pointerId,event);if(activePointers.size===2){panState=undefined;const points=[...activePointers.values()];pinchState={startDistance:pointerDistance(points[0],points[1]),startScale:currentScale};event.preventDefault();document.body.style.setProperty('user-select','none');}});viewer?.addEventListener('pointermove',event=>{if(activePointers.has(event.pointerId))activePointers.set(event.pointerId,event);if(!pinchState||activePointers.size<2)return;const points=[...activePointers.values()];const distance=pointerDistance(points[0],points[1]);if(pinchState.startDistance>0){event.preventDefault();setZoom(pinchState.startScale*distance/pinchState.startDistance);}});viewer?.addEventListener('pointerup',event=>{activePointers.delete(event.pointerId);if(pinchState)endPinch();});viewer?.addEventListener('pointercancel',event=>{activePointers.delete(event.pointerId);if(pinchState)endPinch();});viewer?.addEventListener('lostpointercapture',event=>{activePointers.delete(event.pointerId);if(pinchState)endPinch();});</script>`,
