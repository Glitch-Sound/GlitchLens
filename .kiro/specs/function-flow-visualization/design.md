...
| 9.1 | 固定初期倍率 | VisualizationView | DisplayState | Viewer Flow |
| 9.2 | 拡大と縦横比維持 | VisualizationView | DisplayState | Viewer Flow |
| 9.3 | 縮小と縦横比維持 | VisualizationView | DisplayState | Viewer Flow |
| 9.4 | 倍率範囲の制限 | VisualizationView | ZoomBounds | Viewer Flow |
| 9.5 | リセット | VisualizationView | DisplayState | Viewer Flow |
| 9.6 | ドラッグ移動 | VisualizationView | PanState | Viewer Flow |
| 9.7 | ドラッグ中の選択・スクロール抑止 | VisualizationView | Pointer interaction policy | Viewer Flow |
| 9.8 | 表示操作とMermaid/SVG/source mapの非干渉 | VisualizationView, webviewMermaid.js | Display wrapper contract | Viewer Flow |
| 9.9 | シーケンス図の視覚的余白 | webviewMermaid.js | Mermaid sequence configuration | Mermaid Render Flow |
| 9.10 | 描画失敗時のfallback | VisualizationView, webviewMermaid.js | Fallback view | Viewer Flow |
| 9.11 | マウスホイールズーム | VisualizationView | Wheel zoom policy | Viewer Flow |
| 9.12 | トラックパッド／タッチのピンチズーム | VisualizationView | Pinch gesture policy | Viewer Flow |
| 9.13 | 倍率境界と通常縦スクロールの共存 | VisualizationView | Input arbitration policy | Viewer Flow |
...
Task 11.3では、縦方向の可読性を優先して次の値を採用する。`messageMargin: 70`、`noteMargin: 20`、`diagramMarginY: 10`、`boxMargin: 22`、`boxTextMargin: 12`。横方向に影響する`actorMargin: 40`、`diagramMarginX: 8`、`useMaxWidth: true`は変更しない。
...
| 9.11 | Webview display controller | ホイール方向・量が倍率へ反映され、通常の縦スクロールと競合しないことを確認 |
| 9.12 | Webview display controller | 2本指の距離変化で連続的に倍率が変化し、単一ポインターのパンと競合しないことを確認 |
| 9.13 | Webview display controller | 最小・最大倍率のクランプと入力終了後の通常スクロール復帰を確認 |
...
- `VisualizationView` の表示契約テスト verifies 9.1-9.13: 初期値、ボタン・ホイール・ピンチによる倍率更新、倍率境界、パンとの入力競合、通常縦スクロール、表示ラッパーによる非干渉を検証する。
...
- VisualizationView interaction flow verifies 9.2, 9.3, 9.6, 9.7, 9.11, 9.12, 9.13: WebView上でマウスホイールとピンチをズームとして扱い、ドラッグパン・UI操作・通常縦スクロールと競合しないこと。
