...
- `src/test/flowModelContract.test.ts`、`src/test/typescriptFlowExtractor.test.ts`、`src/test/mermaidRenderer.test.ts` — `self` の左端ライフライン、クラス／インスタンスのタイトル、要求メッセージ、同一 participant の集約、`Unknown`／`Unresolved` のフォールバックを検証する。
...
| 16.1 | 指定関数を最左の `self` ライフラインにする | MermaidRenderer | fixed root participant | Mermaid Render Flow |
...
- 先行する FlowEdge を持たない関数先頭の Call を、`self` root からの要求メッセージとして描画する規則。
