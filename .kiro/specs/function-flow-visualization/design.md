...
- `break-exit` は Break node から、当該ループ後に最初に到達可能な node へ接続する。後続 node が存在しない場合は edge を作らない。
- `continue-loop` は Continue node から、当該 loop node へ接続する。Renderer はこの edge を次の反復として表示し、通常の逐次 `next` edge として解釈しない。
- Renderer が edge の組合せを Mermaid の構造化制御構文へ完全には変換できない場合、render 可能な順序を維持し、`order-uncertain` diagnostic または renderer warning を返す。
...
- participant 名の省略・統合、SourceMap の要素IDや位置の変更。
...
- `FlowCallNode.calleeName` は `map` などのメソッド名を保持し、MermaidRenderer は既存 participant / message 経路で `map` を出力する。
