...
- **In scope**: CodeLens またはカーソル位置からの対象関数特定、TypeScript / JavaScript の静的解析、関数内の処理順序の保持、Call / Branch / Loop / Await / Return / Throw / Try/Catch の表現、指定関数の外部呼び出し元を表す `caller` と対象関数を表す `self` のライフライン、関数内で利用するクラス名またはインスタンス名を他のライフラインのタイトルとして表示すること、Mermaidシーケンス図 表示、Mermaid テキストのコピー、unknown / unresolved と未解析箇所の明示、部分結果表示、シーケンス図のボタン・マウスホイール・トラックパッド／タッチデバイスのピンチによる表示倍率操作。
- **Out of scope**: Sequence Diff、Test Hints、Layer Classification、Architecture Rules、PNG / SVG 出力、Markdown への直接挿入、LLM 連携、実行時トレース、動的呼び出しの完全解決、呼び出し先関数内部の再帰的な深度解析。本仕様の初期言語スコープ外となる TypeScript / JavaScript 以外の言語対応は、恒久的な非対応を意味しない。
- **Adjacent expectations**: この仕様は、ソースコードや解析結果を外部へ送信しないローカルの静的解析体験を前提とし、対象関数の理解に必要な範囲の静的処理フローを表示する。
- **共通基盤の責務**: LanguageAnalyzer、言語非依存の Function Locator contract、Common Flow Model、Mermaid Renderer、WebView、diagnostic、部分結果、安全性、キャッシュ、および Workspace Trust は言語横断の共通基盤として本仕様で扱う。言語別の実装は、これらの公開契約を利用し、言語固有の AST や parser を共通基盤へ伝播させない。
- **言語別拡張の責務**: TypeScript / JavaScript 以外の言語対応は、`python-function-flow-visualization` のような言語別の拡張 spec で定義、設計、実装する。言語別 spec は parser、Language Analyzer、関数 locator、CodeLens 連携、言語固有の構文変換、未解決判定、および非退行テストを責務とする。
- **共通契約の変更**: 追加言語への対応で Common Flow Model、Mermaid 表現、共通 UI、または共通の公開契約の変更が必要になった場合、言語別 spec だけでは完結させず、本仕様の変更として要件・設計・承認の対象にする。
...
### Requirement 6: 解析不能箇所と部分結果の扱い

**Objective:** As a レガシーコードの保守開発者, I want 解析できなかった箇所が明示されるようにしたい, so that 不完全な解析でも安全に読み進められる

#### Acceptance Criteria

1. If 構文エラー、解析器のエラー、または同等の解析不能な状態により呼び出しの解析結果を生成できないが部分結果を表示できる, then the GlitchLens extension shall その箇所を `Unknown` の専用ライフラインと diagnostic で表示する
2. If 明示的に他のクラスまたはインスタンスへの呼び出しであると静的に判断できるが、その呼び出し先の操作を完全には解決できない, then the GlitchLens extension shall その呼び出しを `Unresolved` として表示し、ユーザーが未解決要素を認識できる表示を提供する
...
### Requirement 13: メッセージラベルの簡潔性

**Objective:** As a シーケンス図を読む開発者, I want 呼び出しや戻り値のメッセージラベルを短く意味のある形で確認したい, so that 長い引数、オブジェクトリテラル、式の詳細に視線を奪われず処理の流れを追跡できる

#### Acceptance Criteria

1. When 関数呼び出しを表すメッセージが表示される, the GlitchLens extension shall 呼び出し先の関数名または操作名を中心とした簡潔なラベルを表示し、呼び出し元の式全体や引数の詳細をラベルへ展開しない
2. When return または throw の式に関数呼び出し、オブジェクトリテラル、配列、または複合式が含まれる, the GlitchLens extension shall 式全体を表示せず、処理の種類と主要な呼び出し先または式の概要を含む短いラベルを表示する
3. When メッセージの元となる式が長いまたは入れ子になっている, the GlitchLens extension shall ラベルの長さを一定の表示上限内に収め、上限を超える詳細を省略記号などで示す
4. When メッセージラベルが簡略化される, the GlitchLens extension shall Requirement 17 で定義する return の方向、処理順序、呼び出し先 participant、未解決状態、および元コードへの対応付けを維持する
5. When Mermaid テキストがコピーされる, the GlitchLens extension shall 図に表示された簡潔なメッセージラベルをコピー対象にも反映し、ラベル以外の Mermaid の構造と処理順序を維持する

#### Requirements Change Notes

- 長い return 式やオブジェクトリテラルをメッセージラベルへそのまま展開せず、`return JSON.stringify(...)` のように処理の意味を残した要約表示を求める要件を追加した。
- 呼び出し先、方向、順序、未解決状態、コードジャンプとの対応は維持し、ラベルだけを表示用に簡略化する境界を明文化した。
- Mermaid のコピー結果にも簡潔なラベルを反映し、表示と再利用時でラベルの長さが逆戻りしないことを明確化した。

### Requirement 14: コレクションメソッドの静的呼び出し判定

**Objective:** As a シーケンス図を読む開発者, I want 呼び出し式が返すコレクションに対する標準メソッド呼び出しを未解決扱いせず確認したい, so that `map` などの一般的な処理を不要な unresolved 警告なしに追跡できる

#### Acceptance Criteria

1. When 呼び出し式の戻り値に対して静的にコレクションメソッドと判断できる `map`、`filter`、`flatMap`、`forEach`、`reduce` などが呼び出される, the GlitchLens extension shall そのメソッド呼び出しを resolved として処理し、動的 receiver を理由とする unresolved 警告を表示しない
2. When resolved と判定されたコレクションメソッドが Mermaid シーケンス図へ出力される, the GlitchLens extension shall Requirement 16 で識別したクラス名またはインスタンス名のライフラインへメソッド名をメッセージとして表示し、呼び出し順序を維持する
3. If 動的オブジェクトのメソッド呼び出しまたは計算プロパティによる呼び出しを静的にコレクションメソッドと判断できない, then the GlitchLens extension shall 従来どおり unknown または unresolved として扱い、必要な diagnostic または Note を表示する
4. When コレクションメソッドの静的判定が適用される, the GlitchLens extension shall SourceMap、コードジャンプ、部分解析の完全性、および Mermaid テキストの処理順序を維持する
5. When 静的解析の判定規則が更新される, the GlitchLens extension shall 更新前の判定規則で生成された解析結果を再利用せず、更新後の規則で再解析する

#### Requirements Change Notes

- `findFunctionCandidates(source).map(...)` のように、呼び出し式の結果へ標準コレクションメソッドを適用するケースを、動的 receiver による unresolved 判定から除外する要件を追加した。
- `map` などは Requirement 16 で識別したクラス名またはインスタンス名のライフラインへ送るメッセージとして表示し、`unresolved call` の Note を表示しないことを明文化した。
- `factory.getService(name).run()`、`serviceMap[type].execute()`、計算プロパティ呼び出しなど、静的にコレクションメソッドと判断できない呼び出しは既存の unknown / unresolved 契約を維持する。
- 静的判定規則の更新時に古いキャッシュを再利用しないことを、解析結果の正確性に関わる隣接要件として追加した。

### Requirement 15: 言語横断の実行順と制御フロー契約

**Objective:** As a 複数言語のコードを読む開発者, I want 言語にかかわらず静的に推定できる実行順で処理フローを確認したい, so that 同じ図の読み方で関数を理解できる

#### Acceptance Criteria

1. When 入れ子になった呼び出し式が解析される, the GlitchLens extension shall 構文上の出現順ではなく、静的に推定できる実行順で Call を表現する
2. When `outer(inner())` と同等の式が解析される, the GlitchLens extension shall `inner` を `outer` より前に表現する
3. When ループ内に `break` が含まれる, the GlitchLens extension shall ループ後の次に到達可能な処理へ進む経路を Flow Model で表現する
4. When ループ内に `continue` が含まれる, the GlitchLens extension shall 次の反復へ進む経路を Flow Model で表現する
5. If Mermaid 表示が制御フローの完全な意味論を表現できない, then the GlitchLens extension shall 静的に推定できる処理順を維持し、表現の不確実性を diagnostic で明示する

#### Requirements Change Notes

- 推定呼び出し順を構文走査順ではなく、静的に推定できる実行順として定義した。
- `break` と `continue` の到達可能性を言語横断の Common Flow Model 契約へ昇格した。

### Requirement 16: 指定関数を起点とするライフラインとメッセージ

**Objective:** As a シーケンス図を読む開発者, I want 指定した関数を起点に、その関数内で利用するクラスまたはインスタンスへの要求を追跡したい, so that 関数内の依存先と呼び出し順序を理解できる

#### Acceptance Criteria

1. When 指定した関数の可視化結果が生成される, the GlitchLens extension shall 静的に特定しない外部呼び出し元を最も左の固定ライフライン `caller` として表示し、その右に対象関数自体を共通名 `self` のライフラインとして表示する
2. When 指定した関数内で利用される呼び出し先のクラス名またはインスタンス名を静的に識別できる, the GlitchLens extension shall そのクラス名またはインスタンス名をライフラインのタイトルとして表示する
3. When 指定した関数からクラスまたはインスタンスのライフラインへ呼び出しが表示される, the GlitchLens extension shall そのクラスまたはインスタンスに要求する関数名またはメソッド名を、引数の詳細を含まないメッセージとして表示する
4. When 同じクラスまたはインスタンスに対する複数の呼び出しが表示される, the GlitchLens extension shall それらを同じライフラインへ表示し、異なるクラスまたはインスタンスに属する同名の操作を同じライフラインへ統合しない
5. When 指定した関数内の呼び出しについて、明示的に他のクラスまたはインスタンスへの呼び出しであると静的に判断できず、かつ当該箇所の解析が継続できる, the GlitchLens extension shall その呼び出しを対象関数自身のインスタンスへの呼び出しとして既存の `self` ライフライン上にネストした活性化期間で表示し、`Unknown` または `Unresolved` の専用ライフラインを表示しない
6. If 構文エラー、解析器のエラー、または同等の解析不能な状態により呼び出しの解析結果を生成できないが部分結果を表示できる, then the GlitchLens extension shall 例外として `Unknown` の専用ライフラインと、解析できなかったことを示す diagnostic を表示する
7. When ライフラインとメッセージが Mermaid テキストとして生成またはコピーされる, the GlitchLens extension shall 図とコピー対象で、左端の `caller`、その右の `self`、`caller` から `self` への開始呼び出し、クラス名またはインスタンス名、自己インスタンスとして扱った呼び出し、解析エラー時の `Unknown`、および処理順序を同じ内容で表示する
8. When 指定した関数内の最初の表示可能な呼び出しに先行する表示可能な処理がない, the GlitchLens extension shall その呼び出しを `self` ライフラインからのメッセージとして表示し、関数先頭にあることだけを理由に省略しない
9. When 指定した関数の可視化結果が生成される, the GlitchLens extension shall 関数本体の処理より前に、固定ライフライン `caller` から `self` へ対象関数の開始を示す呼び出しメッセージを一度だけ表示する

#### Requirements Change Notes

- 静的に特定しない外部呼び出し元を最も左の固定 `caller` ライフラインとして追加し、指定した関数自体はその右にある `self` ライフラインとして表示する。対象関数名や呼び出し元の実在する関数名はライフラインのタイトルに表示しない。
- `caller` と `self` の関係を読み取れるよう、関数内の処理に先行して `caller` から `self` への開始呼び出しを一度だけ表示する。呼び出し元の実在名は静的に推測しない。
- その他のライフラインは、指定した関数内で利用されるクラス名またはインスタンス名だけをタイトルに用いる。役割名、パッケージ名、モジュール名、ファイル名は代替タイトルに用いない。
- クラスまたはインスタンスへの要求は、関数名またはメソッド名を引数の詳細なしでメッセージに表示する。明示的に他インスタンスへの呼び出しと判断できない呼び出しは対象関数自身への呼び出しとして `self` に集約し、`Unknown` は解析エラーなどにより解析結果を生成できない例外に限る。
- 関数先頭の最初の表示可能な呼び出しは、先行する処理がない場合でも `self` からのメッセージとして表示し、Mermaid の経路表現上の都合で省略しない。

### Requirement 17: 対象関数の戻り値の送信元と送信先

**Objective:** As a シーケンス図を読む開発者, I want 対象関数の戻り値がどこからどこへ返るかを正しく確認したい, so that 関数内の呼び出し先の応答と対象関数自身の返却を混同せずに処理を理解できる

#### Acceptance Criteria

1. When 指定した関数に `return` が含まれる可視化結果が生成される, the GlitchLens extension shall 戻り値メッセージを `self` から固定ライフライン `caller` へ向かう応答として表示する
2. When `return` の直前にクラス、インスタンス、`Unknown`、または `Unresolved` への通常呼び出しもしくは await を伴う呼び出しがある, the GlitchLens extension shall その呼び出し先を対象関数の戻り値メッセージの送信元として表示しない
3. When `return` を含む関数内呼び出しの活性化期間が表示される, the GlitchLens extension shall 呼び出し先の活性化終了と対象関数から `caller` への戻り値メッセージを区別して表示する
4. When `return` の式が簡略化される, the GlitchLens extension shall `return` と戻り値の概要を一度だけ表示し、Requirement 13 のラベル簡略化規則を適用する
5. If 呼び出し元の実在する関数名、クラス名、モジュール名、またはファイル名を静的に特定できない, the GlitchLens extension shall それらを推測して `caller` のタイトルに使用しない
6. If return を含む解析結果が partial、unknown、または unresolved を含む, the GlitchLens extension shall 表現可能な戻り値メッセージと既存の未解決・部分解析の通知を維持し、呼び出し先を戻り値の送信元として補完しない
7. When return を含む Mermaid テキストが表示、コピー、または元コードとの対応付けに利用される, the GlitchLens extension shall `caller` を含む同一の Mermaid テキスト、return の処理順序、および戻り値に対応する元コードへの対応付けを維持する
8. When 指定した関数に `throw` が含まれる, the GlitchLens extension shall この要件によって既存の throw の表示規則を変更しない

#### Requirements Change Notes

- 従来の `self` 最左固定を、静的に特定しない外部呼び出し元 `caller` とその右の `self` に変更し、`caller` から `self` への開始呼び出しと `self` から `caller` への return を対として表示する。これにより、対象関数の return を関数内の participant の応答と混同せず、関数呼び出し全体を追跡できる。
- `results.append()` の後の `return results` は `results` から `self` への応答ではなく、`self` から `caller` への応答として表示する。同じ規則を通常呼び出し、await、入れ子の呼び出しを含む全対応言語へ適用する。
- 呼び出し先の活性化終了、unknown / unresolved、partial result、throw、元コードとの対応付け、表示とコピーの Mermaid テキスト一致は、戻り値メッセージの送信元・送信先の変更によって失わない。

### Requirement 18: 対象関数自身として扱う呼び出しの簡潔な表示

**Objective:** As a シーケンス図を読む開発者, I want 明示的に他インスタンスと判断できない呼び出しを対象関数自身への呼び出しとして重複したライフラインなしで確認したい, so that 不確実な主体名に図の理解を妨げられず処理を追跡できる

#### Acceptance Criteria

1. When 指定した関数自身を receiver として静的に識別できる、または明示的に他のクラスまたはインスタンスへの呼び出しであると判断できないメソッド呼び出しが可視化される, the GlitchLens extension shall 追加の `self`、`this`、`Unknown`、または `Unresolved` ライフラインをその呼び出しのために表示しない
2. When 指定した関数自身を receiver として識別できる、または明示的に他インスタンスと判断できない通常のメソッド呼び出しが可視化される, the GlitchLens extension shall 既存の `self` ライフライン上にネストした活性化期間と操作名の Note を表示する
3. When 指定した関数自身を receiver として識別できる、または明示的に他インスタンスと判断できない await を伴うメソッド呼び出しが可視化される, the GlitchLens extension shall 既存の `self` ライフライン上にネストした活性化期間と `await <操作名>` の Note を一度だけ表示する
4. When 対象関数自身として扱う呼び出しが入れ子になっている, the GlitchLens extension shall ネストした活性化期間と Note を静的に推定できる実行順で表示する
5. If 構文エラー、解析器のエラー、または同等の解析不能な状態により呼び出しを対象関数自身または他インスタンスのいずれとしても表現できないが部分結果を表示できる, then the GlitchLens extension shall 例外として `Unknown` の専用ライフラインと該当する diagnostic の規則を適用する
6. When 自己呼び出しを含む Mermaid テキストが表示、コピー、または元コードとの対応付けに利用される, the GlitchLens extension shall Note、活性化期間、処理順序、および元コードへの対応付けを同一の Mermaid テキストで維持する
7. When 自己呼び出しと通常の呼び出し、return、throw、または部分解析の結果が同じ図に含まれる, the GlitchLens extension shall `caller`、`self`、return、unknown / unresolved、および既存の活性化期間の表示規則を維持する

#### Requirements Change Notes

- 本要件では、receiver が対象関数自身であることを静的に識別できるメソッド呼び出しに加え、明示的に他インスタンスへの呼び出しと判断できない呼び出しを対象関数自身として扱う。
- 自己呼び出しは別 participant や自己宛てメッセージを追加せず、既存の `self` ライフライン上のネストした活性化期間と Note で表現する。
- receiver を特定できないことだけを理由に `Unknown` を表示しない。解析が継続できる場合は `self` のネストした活性化期間で表現し、構文エラーや解析器エラーなどにより解析結果を生成できない場合だけ `Unknown` と diagnostic を表示する。
