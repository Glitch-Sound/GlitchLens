...
5. If 指定した関数内の呼び出し先についてクラス名またはインスタンス名を静的に識別できない, then the GlitchLens extension shall 解決状態に応じて `Unknown` または `Unresolved` の専用ライフラインへ集約し、推測したクラス名、インスタンス名、モジュール名、またはファイル名をタイトルとして表示しない
6. When ライフラインとメッセージが Mermaid テキストとして生成またはコピーされる, the GlitchLens extension shall 図とコピー対象で、左端の `caller`、その右の `self`、`caller` から `self` への開始呼び出し、クラス名またはインスタンス名、メッセージ、unknown / unresolved 状態、および処理順序を同じ内容で表示する
7. When 指定した関数内の最初の表示可能な呼び出しに先行する表示可能な処理がない, the GlitchLens extension shall その呼び出しを `self` ライフラインからのメッセージとして表示し、関数先頭にあることだけを理由に省略しない
8. When 指定した関数の可視化結果が生成される, the GlitchLens extension shall 関数本体の処理より前に、固定ライフライン `caller` から `self` へ対象関数の開始を示す呼び出しメッセージを一度だけ表示する
...
- クラスまたはインスタンスへの要求は、関数名またはメソッド名を引数の詳細なしでメッセージに表示する。主体を静的に識別できない呼び出しは、推測せず `Unknown` または `Unresolved` に集約する。
...
### Requirement 18: 対象関数自身への呼び出しの簡潔な表示

**Objective:** As a シーケンス図を読む開発者, I want 対象関数自身への呼び出しを重複したライフラインなしで確認したい, so that 同一インスタンス内の処理を過剰な図の要素に妨げられず追跡できる

#### Acceptance Criteria

1. When 指定した関数自身を receiver として静的に識別できるメソッド呼び出しが可視化される, the GlitchLens extension shall 追加の `self`、`this`、`Unknown`、または `Unresolved` ライフラインをその呼び出しのために表示しない
2. When 指定した関数自身への通常のメソッド呼び出しが可視化される, the GlitchLens extension shall 既存の `self` ライフライン上にネストした活性化期間と操作名の Note を表示する
3. When 指定した関数自身への await を伴うメソッド呼び出しが可視化される, the GlitchLens extension shall 既存の `self` ライフライン上にネストした活性化期間と `await <操作名>` の Note を一度だけ表示する
4. When 指定した関数自身への呼び出しが入れ子になっている, the GlitchLens extension shall ネストした活性化期間と Note を静的に推定できる実行順で表示する
5. If receiver を対象関数自身として静的に識別できない呼び出しが可視化される, then the GlitchLens extension shall その呼び出しを自己呼び出しとして表示せず、既存の `Unknown`、`Unresolved`、および該当する diagnostic の規則を維持する
...
- 本要件の自己呼び出しは、receiver が対象関数自身であることを静的に識別できるメソッド呼び出しだけを指す。修飾なしの直接関数呼び出しを自己呼び出しと推測することは含まない。
- 自己呼び出しは別 participant や自己宛てメッセージを追加せず、既存の `self` ライフライン上のネストした活性化期間と Note で表現する。
- 既存の静的解析の不確実性を隠さないため、receiver を特定できない呼び出しには Unknown / Unresolved と diagnostic の表示を引き続き適用する。
