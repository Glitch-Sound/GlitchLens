...
2. When resolved と判定されたコレクションメソッドが Mermaid シーケンス図へ出力される, the GlitchLens extension shall メソッド名を participant およびメッセージとして表示し、呼び出し順序を維持する
...
- `map` などは participant とメッセージへ表示し、`unresolved call` の Note を表示しないことを明文化した。
