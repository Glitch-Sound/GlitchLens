...
- **In scope**: CodeLens またはカーソル位置からの対象関数特定、TypeScript / JavaScript の静的解析、関数内の処理順序の保持、Call / Branch / Loop / Await / Return / Throw / Try/Catch の表現、指定関数を左端の無題ライフラインとして、関数内で利用するクラス名またはインスタンス名を他のライフラインのタイトルとして表示すること、Mermaidシーケンス図 表示、Mermaid テキストのコピー、unknown / unresolved と未解析箇所の明示、部分結果表示、シーケンス図のボタン・マウスホイール・トラックパッド／タッチデバイスのピンチによる表示倍率操作。
...
1. When 指定した関数の可視化結果が生成される, the GlitchLens extension shall その関数自体を最も左のライフラインとして表示し、ライフラインのタイトルを空にする
...
6. When ライフラインとメッセージが Mermaid テキストとして生成またはコピーされる, the GlitchLens extension shall 図とコピー対象で、左端の無題ライフライン、クラス名またはインスタンス名、メッセージ、unknown / unresolved 状態、および処理順序を同じ内容で表示する
7. When 指定した関数内の最初の表示可能な呼び出しに先行する表示可能な処理がない, the GlitchLens extension shall その呼び出しを左端の無題ライフラインからのメッセージとして表示し、関数先頭にあることだけを理由に省略しない

#### Requirements Change Notes

- 指定した関数自体を、図の最も左にある無題のライフラインとして固定する。関数名はライフラインのタイトルに表示しない。
...
- 関数先頭の最初の表示可能な呼び出しは、先行する処理がない場合でも無題 root からのメッセージとして表示し、Mermaid の経路表現上の都合で省略しない。
