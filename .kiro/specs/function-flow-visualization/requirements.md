...
1. When Mermaidシーケンス図が表示される, the GlitchLens extension shall 関数の参加者数、メッセージ数、ラベル長、SVGの自然サイズに左右されない固定された初期表示倍率を使用する
2. When ユーザーが拡大操作を行う, the GlitchLens extension shall シーケンス図を一定の縦横比を維持したまま拡大する
3. When ユーザーが縮小操作を行う, the GlitchLens extension shall シーケンス図を一定の縦横比を維持したまま縮小する
4. The GlitchLens extension shall 拡大縮小の最小倍率と最大倍率を設け、範囲外の倍率を適用しない
5. When ユーザーがリセット操作を行う, the GlitchLens extension shall 初期表示倍率と初期表示位置へ戻す
6. When シーケンス図が表示領域を超える, the GlitchLens extension shall ユーザーが図をドラッグして表示位置を移動できるようにする
7. While ユーザーが図をドラッグしている, the GlitchLens extension shall 意図しないテキスト選択やページ全体のスクロールを発生させない
8. When ユーザーが倍率を変更または図を移動する, the GlitchLens extension shall Mermaidテキストのコピー内容、SourceMap、SVG装飾に影響を与えない
9. When Mermaidシーケンス図が表示される, the GlitchLens extension shall 特に縦方向のメッセージ、participant、activation、制御ブロック間に現状より十分な視覚的余白を提供する
10. If Mermaidの描画に失敗する, then the GlitchLens extension shall 既存のMermaidテキストfallbackを表示し、倍率・移動操作を要求しない
11. When ユーザーがシーケンス図上でマウスホイールを操作する, the GlitchLens extension shall ホイール操作の方向と量に応じて表示倍率を変更し、図の縦横比を維持する
12. When ユーザーがトラックパッドまたはタッチデバイス上でシーケンス図をピンチ操作する, the GlitchLens extension shall ピンチ操作の拡大縮小に応じて表示倍率を連続的に変更し、図の縦横比を維持する
13. While ユーザーがシーケンス図上でズーム操作を行っている, the GlitchLens extension shall 設定された最小倍率と最大倍率を超える表示倍率を適用せず、通常の縦方向スクロールを継続できる状態を保つ
