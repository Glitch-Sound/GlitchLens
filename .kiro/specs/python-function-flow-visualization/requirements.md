...
### Requirement 4: 解析不能箇所と部分結果

**Objective:** As a Python の動的なコードを読む開発者, I want 静的に確定できない箇所を明示した部分結果を確認したい, so that 図の確実性を理解しながら調査を進められる

#### Acceptance Criteria

1. If Python の構文エラー、解析器のエラー、または同等の解析不能な状態により呼び出しの解析結果を生成できないが部分結果を表示できる, the GlitchLens extension shall 例外としてその箇所を `Unknown` の専用ライフラインと対応する diagnostic で表示する
2. When 計算された属性アクセス、添字アクセス、動的な呼び出し可能オブジェクト、または同等の動的構造により明示的に他のクラスまたはインスタンスへの呼び出しであると判断できず、解析が継続できる, the GlitchLens extension shall その呼び出しを対象関数自身のインスタンスへの呼び出しとして既存の `self` ライフライン上にネストした活性化期間で表示する
3. If Python の対象関数の一部だけを解析できる, the GlitchLens extension shall 解析可能な範囲の Flow Model と diagnostic を返し、全体を失敗させない
4. If Python の構文が編集途中で不完全である、または初期スコープ外の構文に遭遇する, the GlitchLens extension shall 可能な範囲の関数候補または解析結果を提供し、呼び出しの解析結果を生成できない箇所には `Unknown` と理由を示す diagnostic を表示する
...
1. When Python の `receiver.operation()` 形式の呼び出しで receiver が `self` 以外の単一の識別子として静的に識別できる, the GlitchLens extension shall TypeScript と同じ静的命名規則に従い、先頭が大文字の receiver をクラス、それ以外の receiver をインスタンスとして、その receiver 名をライフラインのタイトルに表示する
...
4. When Python の直接関数呼び出し、チェーン途中の呼び出し、添字アクセス、動的属性アクセス、またはその他の動的な呼び出しで、明示的に他のクラスまたはインスタンスへの呼び出しであると静的に判断できず、解析が継続できる, the GlitchLens extension shall TypeScript と同じく対象関数自身のインスタンスへの呼び出しとして既存の `self` ライフライン上にネストした活性化期間で表示し、推測したクラス名、インスタンス名、パッケージ名、モジュール名、またはファイル名をタイトルとして表示しない
...
### Requirement 7: Python の明示的な自己呼び出し

**Objective:** As a Python のシーケンス図を読む開発者, I want `self.method()` を対象関数自身への呼び出しとして簡潔に確認したい, so that 同一インスタンス内の処理と外部または未解決の呼び出しを混同せずに追跡できる

#### Acceptance Criteria

1. When Python の対象関数内に `self.method()` 形式の呼び出しが含まれる, the GlitchLens extension shall その呼び出しを対象関数自身への呼び出しとして共通 Requirement 18 の表示規則で表現する
2. When Python の `await self.method()` が可視化される, the GlitchLens extension shall 既存の `self` ライフライン上にネストした活性化期間と `await <操作名>` の Note を一度だけ表示する
3. When Python の `self.method()` が可視化される, the GlitchLens extension shall その呼び出しのために別の `self` participant、通常の receiver participant、`Unknown`、または `Unresolved` participant を表示しない
4. When Python の修飾なし直接呼び出し、チェーン途中の呼び出し、添字アクセス、または動的属性アクセスで receiver を `self` として識別できず、明示的に他インスタンスへの呼び出しとも判断できず、解析が継続できる, the GlitchLens extension shall その呼び出しを対象関数自身のインスタンスへの呼び出しとして既存の `self` ライフライン上にネストした活性化期間で表示する
5. When Python の自己呼び出しを含む Mermaid テキストが表示またはコピーされる, the GlitchLens extension shall 共通 Requirement 18 と同じ Note、活性化期間、処理順序、および元コードへの対応付けを維持する
6. When Python の自己呼び出しが通常の receiver 呼び出し、return、raise、または部分解析の結果と同じ図に含まれる, the GlitchLens extension shall Python 専用の表示形式を追加せず、共通の `caller`、`self`、活性化期間、および未解決表示の規則を維持する
...
- モジュール名、パッケージ名、ファイル名、および enclosing class 名は、主体を静的に識別できない Python 呼び出しの代替ライフライン名に用いない。明示的に他インスタンスと判断できない主体は共通の `self` を用い、構文エラーや解析器エラーなどにより解析結果を生成できない例外だけ `Unknown` を用いる。
...
- 動的な呼び出し先または部分解析でも、明示的に他インスタンスと判断できない限り `self` のネストした活性化期間を用いる。実行時の状態は推測せず、解析結果を生成できない場合だけ `Unknown` と diagnostic を表示する。
- Python の `self.method()` は共通 Requirement 18 の自己呼び出しとして扱い、別 participant を増やさず既存 `self` ライフライン上のネストした活性化期間と Note で表示する。修飾なし直接呼び出しも、明示的に他インスタンスと判断できず解析が継続できる場合は同じ表示を用いる。
