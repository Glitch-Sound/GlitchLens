...
4. If Python の修飾なし直接呼び出し、チェーン途中の呼び出し、添字アクセス、または動的属性アクセスで receiver が `self` として静的に識別できない, then the GlitchLens extension shall その呼び出しを自己呼び出しとして扱わず、既存の Unknown / Unresolved と diagnostic の規則を維持する
...
- Python の `self.method()` は共通 Requirement 18 の自己呼び出しとして扱い、別 participant を増やさず既存 `self` ライフライン上のネストした活性化期間と Note で表示する。修飾なし直接呼び出しは自己呼び出しと推測しない。
