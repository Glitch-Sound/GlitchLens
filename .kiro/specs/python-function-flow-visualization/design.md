...
- **This Spec Owns**: Python の Call / Await / Return / Throw を含む Flow Model fixture、`self` と静的に識別可能な participant の活性化期間、表示入力と Clipboard の完全一致に関する Python 回帰検証。
...
`self` は participant 宣言の後から図の終端まで活性化する。呼び出し先 participant は共通 Renderer が Call、Return、Throw の Flow Model 順序から導出する活性化開始・終了規則に従う。`await` と入れ子 Call は既存の静的実行順を変更せず、動的 receiver、unknown、unresolved、または部分解析では確定できない実行時状態を追加推測しない。
...
| 6.7 | `self` と静的に識別可能な Python participant を含む fixture で、共通活性化表現を検証する。 |
